    ...
    if 'EMnist' in dataset:
        ...
        if 'alpha' in dataset:
            ...
            if 'mtl' in dataset:
                alpha, ratio=dataset_[2], dataset_[3]
                #path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
                path_prefix=os.path.join('data', 'CIFAR100', 'u20c10-mtl-alpha{}-ratio{}'.format(alpha, ratio))
            else:
                print("use cifar100 diri dataset")
                alpha, ratio=dataset_[1], dataset_[2]
                #path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
                path_prefix=os.path.join('data', 'CIFAR100', 'ou20c10-alpha{}-ratio{}'.format(alpha, ratio))
            train_data_dir=os.path.join(path_prefix, 'train')
            test_data_dir=os.path.join(path_prefix, 'test')
            proxy_data_dir = 'data/proxy_data/mnist-n10/'
        else:
            ...
            if 'mtl' in dataset:
                classes = dataset_[2]
                path_prefix = os.path.join('data', 'CIFAR100', 'u20c10-mtl-class{}'.format(classes))
            else:
                print("use cifar100 class2 dataset")
                classes = dataset_[1]
                path_prefix = os.path.join('data', 'CIFAR100', 'u20c10-class{}'.format(classes))
            train_data_dir = os.path.join(path_prefix, 'train')
            test_data_dir = os.path.join(path_prefix, 'test')
            proxy_data_dir = 'data/proxy_data/mnist-n10/'
        ...
        if 'alpha' in dataset:
            ...
            if 'mtl' in dataset:
                alpha, ratio=dataset_[2], dataset_[3]
                #path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
                path_prefix=os.path.join('data', 'CIFAR10', 'u20c10-mtl-alpha{}-ratio{}'.format(alpha, ratio))                
            else:
                alpha, ratio=dataset_[1], dataset_[2]
                #path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
                path_prefix=os.path.join('data', 'CIFAR10', 'ou20c10-alpha{}-ratio{}'.format(alpha, ratio))
            train_data_dir=os.path.join(path_prefix, 'train')
            test_data_dir=os.path.join(path_prefix, 'test')
            proxy_data_dir = 'data/proxy_data/mnist-n10/'
        else:
            ...
            if 'mtl' in dataset:
                ...
                classes = dataset_[1]
                path_prefix = os.path.join('data', 'CIFAR10', 'u20c10-class{}'.format(classes))
            train_data_dir = os.path.join(path_prefix, 'train')
            test_data_dir = os.path.join(path_prefix, 'test')
            proxy_data_dir = 'data/proxy_data/mnist-n10/'
...
def read_user_data_contrastive(index, data, dataset='', count_labels=False):
    ...
    X_test1, X_test2, y_test = convert_data_contrastive(test_data['x1'], train_data['x2'], test_data['y'], dataset=dataset)
