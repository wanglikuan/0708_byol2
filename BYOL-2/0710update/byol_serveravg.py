...
from flcore.clients.byol_clientavg import clientBYOL
from flcore.servers.serverbase import Server
#from utils.data_utils import read_client_data
from utils.model_utils import read_data, read_user_data, read_user_data_contrastive #added

from threading import Thread
import time


class FedBYOL(Server):
    def __init__(self, device, dataset, algorithm, model, batch_size, learning_rate, global_rounds, local_steps, join_clients,
        ...
        for i, train_slow, send_slow in zip(range(self.num_clients), self.train_slow_clients, self.send_slow_clients):
            ...
            id, train , test = read_user_data_contrastive(i, data, dataset=self.dataset) #added
            client = clientBYOL(device, i, train_slow, send_slow, train, test, model, batch_size, learning_rate, local_steps)
    ...
    def train(self):
        for i in range(self.global_rounds+1):
            ...
            #self.aggregate_parameters() ### 聚合参数 #####################
